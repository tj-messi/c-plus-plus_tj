...
struct tj_time *tj_time_convert(int input_time)
{
	...
	result={1970,1,1,0,0,0};//初始化 
	int pyear_seconds=60*60*24*365;
	int ryear_seconds=60*60*24*366;
	int month_seconds1[13] = { 0,2678400,2419200,2678400,2592000,2678400,2592000,2678400, 2678400,2592000,2678400,2592000,2678400 };
	int month_seconds2[13] = { 0,2678400,2419200+60*60*24,2678400,2592000,2678400,2592000,2678400, 2678400,2592000,2678400,2592000,2678400 };
	int day_seconds=60*60*24;
	int hour_seconds=60*60;
	int min_seconds=60;
	input_time+=8*60*60;
	while((check_year(result.tj_year)&&input_time>=ryear_seconds)||(!check_year(result.tj_year)&&input_time>=pyear_seconds))
	{
		if(check_year(result.tj_year))
		{
			input_time-=ryear_seconds;
			result.tj_year++;
		}
		else
		{
			input_time-=pyear_seconds;
			result.tj_year++;
		}
	}
	while((check_year(result.tj_year)&&input_time>=(month_seconds2[result.tj_month]))||(!check_year(result.tj_year)&&input_time>=(month_seconds1[result.tj_month])))
	{
		if(check_year(result.tj_year))
		{
			input_time-=month_seconds2[result.tj_month];
			result.tj_month++;
		}
		else
		{
			input_time-=month_seconds1[result.tj_month];
			result.tj_month++;
		}
	}
